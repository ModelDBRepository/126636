...
%The model is set up to run 10 tracks for a simultaneously gated tone and noise, 300 msec in duration 4900-Hz bandwidth.

%%%%%%%%%%%%%%%%%%%%  Stimulus Parameters %%%%%%%%%%%%%%%%%%%%%
fs = 50000;  %sampling rate
tdres = 1/fs; % time-doimain resolution

bw = 4900;   %bandwidth of stimulus noise 
lofreq = 100; % low frequency bound of stimulus noise
No = 28.1;     % spectrum level of noise
 tmp = 0.3410; %average RMS of this noise over a few reps. This should be recomputed when changing noise parameters
snr  =  45;    % signal-to-noise ratio (dB SPL above spectrum level of noise)
centerfreq = 500;  % Tone frequency
dur = 0.300; % stimulus duration in seconds 
ramplength = 0.01; % onset/offset ramp duration
delay_tone = 1; % shortens tone to 200 msec and delays it wrt noise onset by 50 msec


%%%%%%%%%%%%%%%%%% Model parameters %%%%%%%%%%%%%%%%%%%%%%%%%%
template_snr = 45; % dB above spectrum level for template 
template_reps = 10; % if frozen temp ==1, then template_reps are 25 noises
n_track = 5; %number of tracks to run
...
tone_ramp = [ zeros(1,onenvdur*fs) onramp ones(1,tonedur*fs-length([onramp offramp]))  offramp 0 zeros(1,floor(offenvdur*fs)) ]; % Envelope for stimulus (i.e. on/off ramps)
...
for i_track = 1:n_track;
    ...
    for i = 1:template_reps
        ...
        plot(template_adap_n);
        template_adap_tin = nlalmex(template_fscaled2_tin',fs)';
        
        template_fadap_n = filter(b8,a8,template_adap_n); %lowpass filter again at 8 Hz
    ...
    while cur_revs ~= step_revs(i_step_rev);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Optimal Detector
        %The output of the optimal detector first computes the difference between the current
        ...
        % The normalization effectively get rid of the second term which is related to energy
        ...
        %     % compute using Torsten's decision device with internal noise see Torsten's paper for more details
...
xlabel('Time [sec]')
ylabel('model units')
title('Normalized Difference Template (from final track)')
figure
hold on
plot(t,template_n)
